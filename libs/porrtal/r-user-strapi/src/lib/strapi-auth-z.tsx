...
export interface StrapiAuthZProps {
  children?: React.ReactNode;
}

export function StrapiAuthZ(props: StrapiAuthZProps) {
  const authN = useAuthN();
  const authZsDispatch = useAuthZsDispatch();
  const shellDipatch = useShellDispatch();

  useEffect(() => {
    console.log(`Strapi auth z: ${authN?.authNState}`, authN);
    switch (authN?.authNState) {
      ...
      case 'authenticated': {
        const roles = (
          (authN?.claims?.porrtal_roles ?? []) as { name: string }[]
        ).map((role) => role.name);
